...
export class VariantsMock {
  ...
  private readonly setupRenewVapidKeysMock = (id: string) => {
    nock(this.basePath)
      .put(
        `/rest/applications/${this.app.pushApplicationID}/web_push/${id}/renew`
      )
      .reply(() => {
        const app = this.datastore.getApp(this.app.pushApplicationID);
        const variant = app?.variants?.find(
          vv => vv.variantID === id && vv.type === 'web_push'
        ) as WebPushVariant;
        if (!app || !variant) {
          return [404];
        }
  ...
  private readonly setupUpdateVariantMock = (v: Variant) => {
    nock(this.basePath)
      ...
      .reply((url, body) => {
        const app = this.datastore.getApp(this.app.pushApplicationID);
        const variantIndex = app?.variants?.findIndex(
          vv => vv.variantID === v.variantID && vv.type === v.type
        );
  ...
  private readonly setupResetSecretMock = (v: Variant) => {
    nock(this.basePath)
      .put(
        `/rest/applications/${this.app.pushApplicationID}/${v.type}/${v.variantID}/reset`
      )
      .reply(() => {
        const app = this.datastore.getApp(this.app.pushApplicationID);
        const variantIndex = app?.variants?.findIndex(
