...
export const getAllApplications = (): PushApplication[] =>
  datastore.getAllApps();
export const getAppMetrics = (
  ...
  datastore.getAppMetrics(appId, page, perPage, sort, search);
// variants mocks
...
): Variant => applicationMocks!.createVariant(appId, name, variantType, def);
...
export const createMetrics = ({
  ...
  minCount = 3,
  maxCount = 50,
}: CreateMetricsParams): void => {
  const metricsCount =
    count === -1 ? Math.ceil(Math.random() * maxCount) + minCount : count;
