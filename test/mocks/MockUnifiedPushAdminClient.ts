...
const createApplicationMock = jest.fn(
  (name: string): PushApplication => {
    return { id: 'new-app-id', pushApplicationID: 'new-app-push-id', name } as PushApplication;
  }
);
const findVariantsMock = jest.fn(
  (appId: string, filter?: Record<string, string>): Variant[] =>
    applyVariantFilter(mockData.find(app => app.pushApplicationID === appId)!.variants!, filter) || []
);
...
const deleteVariantsMock = jest.fn((appId: string, filter: Record<string, string>): Variant[] => {
